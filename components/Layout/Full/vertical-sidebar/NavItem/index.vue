        ...
        :to="item.type === 'external' ? '' : '/admin' + item.to" :href="item.type === 'external' ?  item.to : ''" rounded
            ...
            <template v-slot:prepend>
                <div class="navbox  bg-hover-primary" >
                    <span class="icon-box">
                        <Icon :item="item.icon" :level="level"  class="position-relative z-index-2 texthover-primary" />
                    </span>
                </div>
            </template>
            <v-list-item-title class="text-subtitle-1 font-weight-medium" color="primary">{{item.title }}</v-list-item-title>
            <!---If Caption-->
